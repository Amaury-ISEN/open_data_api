...
def filtrer():
    ...
    if reg == '0':  # Si l'utilisateur choisit toutes les régions :
        ...
        donnees_gaz = recup_data(fil="gaz", reg_code=reg_code)
        donnees_gaz = [
            donnees_gaz]  # Passage de la donnée sous forme de liste pour l'injecter plus facilement dans highchart
        donnees.append(donnees_gaz)

        # Récupération des données filière élec pour la région choisie
        donnees_elec = recup_data(fil="electricite", reg_code=reg_code)
        donnees_elec = [
            donnees_elec]  # Passage de la donnée sous forme de liste pour l'injecter plus facilement dans highchart
        donnees.append(donnees_elec)

        donnees.append(
            " ")  # Donnees[4] sert à contenir les noms de régions à afficher en axe y, ici " " permet de ne rien afficher.
