from flask import Flask, render_template, redirect,url_for, request as flaskrequest
...
@app.route('/zigouiller_document', methods=["POST"])
def zigouiller_document():

    recordid = flaskrequest.form["recordid"]
    cible_a_zigouiller = requests.delete("http://127.0.0.1:8000/items/delete/"+recordid)
    print(recordid)
    flash("Document supprimé")
    return redirect(url_for("index"))
    
    #return render_template("suppression.html",cible_a_zigouiller)
...
@app.route('/modifier_document', methods=["POST"])
def modifier_document():

    recordid = flaskrequest.form["recordid1"]
    champs = flaskrequest.form["champs"]
    donnee = flaskrequest.form["donnee"]
    cible_a_modifier = requests.put(f"http://127.0.0.1:8000/items/conso/update/"+recordid+"/"+champs+"/"+donnee)
    print(recordid)
